                                ...
                                const char *identifier)
{
    ...
    (void)handle;
    /* The identifier is the only stable name a toplevel has here. It is an
     * opaque string chosen by the compositor, not a number: unlike an X11 XID
     * or a compositor object id there is nothing to derive from it, and it is
     * the handle a caller must pass back. */
    if (toplevel != NULL)
